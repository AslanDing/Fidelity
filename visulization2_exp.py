    ...
    for value_count in dict_name.keys():
        ...
        for key in data_dict.keys():
            ...
            if remove_axis in x_ticks and add_axis in x_ticks:
                value = data[value_count]
                matrix[dict_ratio[remove_axis],dict_ratio[add_axis]] = math.exp(value)
        ...
        # plt.title('%s'%name)
        if 'plus' in name:
            plt.savefig(save_path +'_%s_exp.png'%(name))
            plt.savefig(save_path +'_%s_exp.pdf'%(name))
        elif 'minus' in name:
            plt.savefig(save_path +'_%s_exp.png'%(name))
            plt.savefig(save_path +'_%s_exp.pdf'%(name))
        else:
...
def visualization_fid_ori_ratio(data_dict,save_path=None):
    ...
    for value_count in dict_name.keys():
        ...
        # plt.title('%s'%name)
        # plt.show()
        plt.savefig(save_path+'%s_exp.png'%name)
        plt.savefig(save_path+'%s_exp.pdf'%name)
        plt.close()
