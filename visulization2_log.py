    ...
    for value_count in dict_name.keys():
        ...
        for key in data_dict.keys():
            ...
            if remove_axis in x_ticks and add_axis in x_ticks:
                value = data[value_count]
                matrix[dict_ratio[remove_axis],dict_ratio[add_axis]] = value*100

        if matrix.min()<0:
            matrix = matrix
        else:
            matrix = np.log(matrix+1E-9)
        ...
        # plt.title('%s'%name)
        if 'plus' in name:
            plt.savefig(save_path +'_%s_log.png'%(name))
            plt.savefig(save_path +'_%s_log.pdf'%(name))
        elif 'minus' in name:
            plt.savefig(save_path +'_%s_log.png'%(name))
            plt.savefig(save_path +'_%s_log.pdf'%(name))
        else:
...
def visualization_fid_ori(data_dict,save_path=None):
    ...
    for value_count in dict_name.keys():
        ...
        for key in data_dict.keys():
            ...
            if remove_axis in x_ticks and add_axis in x_ticks:
                value = data[value_count]
                matrix[dict_ratio[remove_axis],dict_ratio[add_axis]] = math.log(value)
    ...
    for value_count in dict_name.keys():
        ...
        for key in data_dict.keys():
            ...
            if remove_axis in x_ticks and add_axis in x_ticks:
                value = data[value_count]
                # if value<1E-9:
                #     matrix[dict_ratio[remove_axis], dict_ratio[add_axis]] = math.log( value + 1E-9)
                # else:
                matrix[dict_ratio[remove_axis],dict_ratio[add_axis]] = value*100
...
if __name__ == "__main__":
    ...
    for path in namelist:
        if 'syn1' in path or 'syn2' in path:
            pass
        else:
            continue
        data = np.load(path, allow_pickle=True).item()
