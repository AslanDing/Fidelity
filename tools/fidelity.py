...
def edit_distance_gt_k_plus(type, model_to_explain, nodes, edges, explain_edges, weight, label, index, k=1,
                                pred_label=False,
                                undirect=True,
                                max_length=100,
                                r_shuffle=True):
    ...
    undirect: undirected graph
    max_length: sample numbers
    r_shuffle: shuffle during calculation
    ...
    if undirect:
        ...
        if len(explain_list)>max_length:
            random.shuffle(explain_list)
        explain_list = explain_list[:max_length]
        if len(non_explain_list)>max_length:
            random.shuffle(non_explain_list)
        non_explain_list = non_explain_list[:max_length]
        ...
        if len(explain_indexs)>max_length:
            random.shuffle(explain_indexs)
        explain_indexs = explain_indexs[:max_length]
        if len(non_explain_indexs)>max_length:
            random.shuffle(non_explain_indexs)
        non_explain_indexs = non_explain_indexs[:max_length]
    ...
    def cal_fid_embedding_plus(explain_indexs_combin):
        ...
        if r_shuffle:
            ...
            # pass
        length = min(explaine_length,max_length) #int(torch.sum(explain).item())
    ...
    fid_plus_mean,fid_plus_label_mean =  cal_fid_embedding_plus(explain_indexs_combin)
    return fid_plus_mean,fid_plus_label_mean,explain_weights

def edit_distance_gt_k_minus(type, model_to_explain, nodes, edges, explain_edges, weight, label, index, k=1,
                                pred_label=False,
                                undirect=True,
                                max_length=100,
                                r_shuffle=True):
    ...
    def cal_fid_embedding_minus(non_explain_indexs_combin):
        ...
        length = min(non_explaine_length, max_length)  # int(torch.sum(explain).item())
    ...
    fid_minus_mean, fid_minus_label_mean = cal_fid_embedding_minus(non_explain_indexs_combin)
    return fid_minus_mean, fid_minus_label_mean, explain_weights
