...
def remap_ed_k():
    ...
    for i in tqdm(range(len(graphs))):  # len(graphs) indices
        ...
        matrix_1 = graph[1]
        # weights = label[1]
        if matrix_0.shape[0]%2 == 1:
            print("odd edges")
            reset_n = int(np.sum(weights))
            # if matrix_0
            if matrix_0[-reset_n]==matrix_1[-reset_n]:
                weights[-reset_n]=0
            else:
        ...
        for list_t in edit_distance_lists:
            for weight in list_t:
                ...
                if matrix_0.shape[0] % 2 == 1:
                    w_list = list(weight)
                    w_list[-reset_n] = 1
                    edit_distance_rt[remove_number, add_number].append(w_list)
                else:
    ...
    np.save('./data/%s_sample_weights_remap'%dataset_name, all_edit_distance_remap)
